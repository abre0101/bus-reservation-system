...
const AdminPayments = () => {
  ...
  const [refundStats, setRefundStats] = useState({ total_refunds: 0, refunded_bookings_count: 0 })
  ...
  const fetchPayments = async () => {
    try {
      ...
      if (data.refund_stats) {
        console.log('💰 Refund stats:', data.refund_stats)
        setRefundStats(data.refund_stats)
      } else {
        console.warn('⚠️ No refund_stats in response')
      }
    } catch (error) {
  ...
  const stats = {
    ...
    refunded: refundStats.total_refunds || 0,
    failed: filteredPayments.filter(p => p.status === 'failed').length,
    netRevenue: (filteredPayments.filter(p => p.status === 'success').reduce((sum, p) => sum + (p.amount || 0), 0) - 
                (refundStats.total_refunds || 0))
  }
    ...
    <div className="space-y-6 p-6 bg-gray-50 min-h-screen">
      ...
      <div className="grid grid-cols-1 md:grid-cols-5 gap-6">
        <div className="bg-gradient-to-br from-purple-500 to-purple-600 rounded-2xl shadow-lg p-6 text-white transform hover:scale-105 transition-transform">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-4xl font-bold mt-2">{stats.netRevenue.toLocaleString()}</p>
        ...
        <div className="bg-gradient-to-br from-blue-500 to-blue-600 rounded-2xl shadow-lg p-6 text-white transform hover:scale-105 transition-transform">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-blue-100 text-sm font-medium uppercase tracking-wide">Refunded</p>
              <p className="text-4xl font-bold mt-2">{stats.refunded.toLocaleString()}</p>
              <p className="text-blue-100 text-sm mt-1">ETB ({refundStats.refunded_bookings_count || 0} bookings)</p>
            </div>
