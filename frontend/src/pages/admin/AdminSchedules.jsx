...
const AdminSchedules = () => {
  ...
  const fetchBuses = async () => {
    try {
      ...
      console.log('🚌 Buses array:', response.buses)
      console.log('🚌 Buses count:', response.buses?.length || 0)
      ...
      if (activeBuses.length === 0) {
        console.warn('⚠️ No active buses available')
      }
    } catch (error) {
  ...
  const fetchTariffRates = async () => {
    try {
      const response = await adminService.getCurrentTariffRates()
  ...
  const handleSaveSchedule = async () => {
    try {
      ...
      if (showEditModal && selectedSchedule) {
        await adminService.updateSchedule(selectedSchedule._id, formData)
  ...
  const handleConfirmEmergencyCancel = async () => {
    ...
    try {
      const response = await adminService.emergencyCancelSchedule(scheduleToCancel._id, {
        reason: cancelReason,
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
        ...
        <div className="bg-white rounded-lg shadow-md p-6 border-l-4 border-purple-500">
          <p className="text-gray-600 text-sm font-medium">Departed</p>
          <p className="text-3xl font-bold text-purple-600">{stats.departed}</p>
        </div>
          ...
          <table className="w-full">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {filteredSchedules.map((schedule) => {
                ...
                const availableSeats = schedule.available_seats || (totalSeats - bookedSeats)
      ...
      {(showAddModal || showEditModal) && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-0">
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="col-span-2">
                  ...
                  {buses.length === 0 && (
                    <p className="text-xs text-red-600 mt-1">⚠️ No buses available. Please add buses first.</p>
                  )}
